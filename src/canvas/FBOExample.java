...
public class FBOExample {
    ...
    public void initGL() {
        ...
        GLU.gluPerspective (45.0f, ((float)WIDTH/(float)HEIGHT), 1.0f, -1f);        // Calculate The Aspect Ratio Of The Window 
...
//        glHint (GL_PERSPECTIVE_CORRECTION_HINT, GL_NICEST);         // Set Perspective Calculations To Most Accurate
        ...
        if (!GLContext.getCapabilities().GL_EXT_framebuffer_object) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 1280, 720, 0,GL_RGBA, GL_INT, (java.nio.ByteBuffer) null);  // Create the texture data
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,GL_COLOR_ATTACHMENT0_EXT,GL_TEXTURE_2D, colorTextureID, 0); // attach it to the framebuffer
     
     
            // initialize depth renderbuffer
            glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, depthRenderBufferID);                // bind the depth renderbuffer
            glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL14.GL_DEPTH_COMPONENT24, 1280, 720); // get the data space for it
    ...
    public void renderGL() {
        ...
        glViewport (0, 0, 1280, 720);                                    // set The Current Viewport to the fbo size
 
        Texture t = null;
        try {
			t = TextureLoader.getTexture("PNG", ResourceLoader.getResourceAsStream("img/bg.png"));
		} catch (IOException e) {
        ...
        glBindTexture(GL_TEXTURE_2D, t.getTextureID());                                // unlink textures because if we dont it all is gonna fail
...
//        glColor3f(1,1,0);                                               // set color to yellow
        ...
        glLoadIdentity ();                                              // Reset The Modelview Matrix
        glTranslatef (0.0f, 0.0f, -3f);                               // Translate 6 Units Into The Screen and then rotate
//        glRotatef(angle,0.0f,1.0f,0.0f);
...
//        glColor3f(1,1,1);                                               // set the color to white
    ...
    //rendering the box with a texture
    public void drawBox() { 
    ...
    public void drawOnTexture() { 
        glBegin(GL_QUADS);
            // For each tile, we will do one of these
