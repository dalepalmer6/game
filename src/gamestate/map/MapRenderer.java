...
public class MapRenderer extends DrawableObject implements Drawable{
	...
	public MapRenderer(Map m, Camera c, SystemState s) {
...
//				m.renderTile(x + (i-1)*TILE_SIZE, y + (j-1)*TILE_SIZE,TILE_SIZE,TILE_SIZE, tileMap.getTile(0).getInstance(0).getDx(),tileMap.getTile(0).getInstance(0).getDy(),tileMap.getTile(0).getInstance(0).getDw(),tileMap.getTile(0).getInstance(0).getDh());
	...
	public void drawTileBase(MainWindow m, int x, int y, Tile t, int instance) {
		Tile.initDrawTiles(m,map.getTileset());
		m.renderTilesToFrameBuffer(-camera.getX()%TILE_SIZE + this.x + (x-1)*TILE_SIZE, -camera.getY()%TILE_SIZE+ this.y + (y-1)*TILE_SIZE,TILE_SIZE,TILE_SIZE, t.getDx(instance),t.getDy(instance),t.getDw(instance),t.getDh(instance),false);
	}
	...
	public void drawTile(MainWindow m, int x, int y, Tile tbg, int instancebg, Tile tfg, int instancefg) {
		...
		for (int i = 0; i < 4; i++) { //y
			for (int j = 0; j < 4; j++) { //x
				...
				if (drawBG)
					m.renderTilesToFrameBuffer(-camera.getX()%TILE_SIZE + this.x + (x-1)*TILE_SIZE + (j*8*4), -camera.getY()%TILE_SIZE+ this.y + (y-1)*TILE_SIZE + (i*8*4),TILE_SIZE/4,TILE_SIZE/4, tbg.getDx(instancebg) + (j*8),tbg.getDy(instancebg) + (i*8),8,8,false);
				if (drawFG)
					m.renderTilesToFrameBuffer(-camera.getX()%TILE_SIZE + this.x + (x-1)*TILE_SIZE + (j*8*4), -camera.getY()%TILE_SIZE+ this.y + (y-1)*TILE_SIZE + (i*8*4),TILE_SIZE/4,TILE_SIZE/4, tfg.getDx(instancefg) + (j*8),tfg.getDy(instancefg) + (i*8),8,8,false);
			}
		}
...
//		system.map.setChangeMap("BG");
//		getAreaOfInterest();
//		drawTilesBG(m);
//		system.map.setChangeMap("FG");
//		getAreaOfInterest();
//		drawTiles(m);
	}
